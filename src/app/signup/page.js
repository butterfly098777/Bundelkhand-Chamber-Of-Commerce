...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      setAlert({ show: true, message: 'Passwords do not match', isError: true });
      setTimeout(() => setAlert({ show: false, message: '', isError: false }), 3000);
      return;
    }

    try {
      console.log('Sending fetch to http://localhost:5000/signup'); // Debug log
      ...
      if (res.ok) {
        setAlert({ show: true, message: 'User registered successfully!', isError: false });
        setFormData({ name: '', email: '', password: '', confirmPassword: '' });
      } else {
        setAlert({ show: true, message: data.message || 'Signup failed', isError: true });
      }
      setTimeout(() => setAlert({ show: false, message: '', isError: false }), 3000);
    } catch (err) {
      console.error('Fetch error:', err.message, err.stack); // Debug log
      setAlert({ show: true, message: `Failed to connect to server: ${err.message}`, isError: true });
      setTimeout(() => setAlert({ show: false, message: '', isError: false }), 3000);
    }
  };
